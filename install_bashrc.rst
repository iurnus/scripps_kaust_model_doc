...
SKRIPS v2.0.3 is available from Github::

  https://github.com/iurnus/scripps_kaust_model/releases/tag/v2.0.3
...
model and set up the environmental variables in the bashrc file.

Install step 1.1: Download SKRIPS version from Github::

  # I am using my home folder as an example
  cd $HOME
  wget https://github.com/iurnus/scripps_kaust_model/archive/refs/tags/v2.0.3.zip
  unzip v2.0.3.zip
  mv scripps_kaust_model-2.0.3 scripps_kaust_model
...
In the ``~/.bashrc_skrips`` file, we have::
  ...
  export WRF_DIR=$SKRIPS_DIR/WRFV471_AO/
  export WPS_DIR=$SKRIPS_DIR/WPSV45/
  export PWRF_DIR=$SKRIPS_DIR/PWRFV471_AO/
